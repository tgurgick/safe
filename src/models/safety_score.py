...
class SafetyScore(BaseModel):
    ...
    violence_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Violence detection score"
    )
    hate_speech_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Hate speech detection score"
    )
    bias_score: float = Field(default=0.0, ge=0.0, le=1.0, description="Bias detection score")
    privacy_score: float = Field(default=0.0, ge=0.0, le=1.0, description="Privacy violation score")
    factual_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Factual accuracy score (lower is better)"
    )

    # Overall safety score
    overall_score: float = Field(default=0.0, ge=0.0, le=1.0, description="Overall safety score")

    # Confidence in the assessment
    confidence: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Confidence in the safety assessment"
    )

    # Safety level classification
    safety_level: SafetyLevel = Field(
        default=SafetyLevel.SAFE, description="Safety level classification"
    )

    # Additional metadata
    flagged_keywords: list[str] = Field(
        default_factory=list, description="Keywords that triggered safety flags"
    )
    reasoning: str = Field(default="", description="Reasoning for the safety assessment")
    ...
    @classmethod
    def create_unsafe_score(
        cls, dimension: str, score: float, reasoning: str = ""
    ) -> "SafetyScore":
        ...
        return cls(**scores.get(dimension, {}), reasoning=reasoning)
