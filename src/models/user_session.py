...
class UserSession(BaseModel):
    ...
    flagged_requests: int = Field(
        default=0,
        description="Number of requests flagged for safety concerns"
    )
    ...
    rate_limit_exceeded: bool = Field(
        default=False,
        description="Whether user has exceeded rate limits"
    )
    ...
    safety_level: str = Field(
        default="moderate",
        description="User's preferred safety level"
    )
    custom_rules: Dict[str, Any] = Field(
        default_factory=dict,
        description="User-specific safety rules"
    )
