...
class SafetyReport(BaseModel):
    ...
    # Basic information
    content_id: str = Field(default="", description="Unique identifier for the content")
    content_type: str = Field(
        default="text", description="Type of content (text, image, audio, etc.)"
    )

    # Safety assessment
    is_safe: bool = Field(default=True, description="Overall safety determination")
    safety_score: SafetyScore = Field(description="Detailed safety scoring")

    # Processing details
    processing_time_ms: float = Field(
        default=0.0, description="Time taken to process content (milliseconds)"
    )
    filters_applied: List[str] = Field(
        default_factory=list, description="List of filters that were applied"
    )
    ...
    # Metadata
    timestamp: str = Field(default="", description="Timestamp of the safety assessment")
    version: str = Field(default="1.0", description="Version of the safety layer used")
    ...
    def get_reasoning(self) -> str:
        ...
        if not self.is_safe:
            primary_concern = self.get_primary_concern()
            return (
                f"Content flagged as unsafe due to {primary_concern}. {self.safety_score.reasoning}"
            )
        else:
    ...
    def create_safe_report(
        cls, content_id: str = "", processing_time: float = 0.0
    ) -> "SafetyReport":
