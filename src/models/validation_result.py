...
class ValidationResult(BaseModel):
    ...
    # Basic validation
    is_valid: bool = Field(default=True, description="Whether the input passed basic validation")
    is_safe: bool = Field(default=True, description="Whether the input is safe for processing")

    # Input details
    original_text: str = Field(default="", description="Original input text")
    sanitized_text: str = Field(default="", description="Sanitized input text")

    # Validation details
    validation_errors: List[str] = Field(
        default_factory=list, description="List of validation errors"
    )
    safety_score: SafetyScore = Field(description="Safety assessment of the input")

    # Rate limiting
    rate_limit_exceeded: bool = Field(default=False, description="Whether rate limit was exceeded")
    ...
    processing_time_ms: float = Field(
        default=0.0, description="Time taken to validate input (milliseconds)"
    )
    filters_applied: List[str] = Field(
        default_factory=list, description="List of validation filters applied"
    )
    ...
    def create_valid_result(
        cls, text: str, sanitized: str = "", processing_time: float = 0.0
    ) -> "ValidationResult":
    ...
    def create_invalid_result(
        cls, text: str, errors: List[str], processing_time: float = 0.0
    ) -> "ValidationResult":
    ...
    @classmethod
    def create_unsafe_result(
        cls, text: str, dimension: str, score: float, reasoning: str, processing_time: float = 0.0
    ) -> "ValidationResult":
    ...
    def create_rate_limited_result(
        cls, text: str, reset_time: int, processing_time: float = 0.0
    ) -> "ValidationResult":
