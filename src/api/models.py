...
class ValidateRequest(BaseModel):
    ...
    safety_level: Optional[SafetyLevelEnum] = Field(
        SafetyLevelEnum.MODERATE,
        description="Safety level to use for validation"
    )
...
class SafetyScoreResponse(BaseModel):
    ...
    flagged_keywords: List[str] = Field(default_factory=list, description="Keywords that triggered flags")
...
class ValidateResponse(BaseModel):
    ...
    filters_applied: List[str] = Field(default_factory=list, description="Filters that were applied")
...
class ProcessResponse(BaseModel):
    ...
    input_safety_score: Optional[SafetyScoreResponse] = Field(None, description="Input safety scores")
    output_safety_score: Optional[SafetyScoreResponse] = Field(None, description="Output safety scores")


class StatsResponse(BaseModel):
    """Response model for safety statistics."""
    content_filter_stats: Dict[str, Any] = Field(..., description="Content filter statistics")
    prompt_injection_stats: Dict[str, Any] = Field(..., description="Prompt injection detector stats")
