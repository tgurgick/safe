...
class PromptInjectionDetector:
    ...
    def __init__(self, config: SafetyConfig):
        ...
        self.config = config
        self.stats = {"total_checks": 0, "injection_detected": 0, "false_positives": 0}
    ...
    def _calculate_injection_score(
        self, text: str, injection_matches: List[str], whitelist_matches: List[str]
    ) -> float:
    ...
    def _classify_injection_type(self, matches: List[str]) -> str:
        ...
        system_manipulation = sum(
            1
            for match in matches
            if any(word in match.lower() for word in ["system", "ignore", "forget"])
        )
        role_manipulation = sum(
            1
            for match in matches
            if any(word in match.lower() for word in ["role", "job", "act as"])
        )
        safety_bypass = sum(
            1
            for match in matches
            if any(word in match.lower() for word in ["safety", "bypass", "disable"])
        )
        output_manipulation = sum(
            1
            for match in matches
            if any(word in match.lower() for word in ["output", "respond", "answer"])
        )
