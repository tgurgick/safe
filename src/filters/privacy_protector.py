...
class PrivacyProtector:
    ...
    def _init_privacy_patterns(self):
        ...
        self.sensitive_keywords = {
            'password', 'secret', 'private', 'confidential', 'secure',
            'token', 'key', 'credential', 'login', 'username', 'account',
            'social_security', 'ssn', 'credit_card', 'bank_account',
            'address', 'phone', 'email', 'birth_date', 'passport'
        }
    ...
    def protect_privacy(self, text: str) -> Dict[str, Any]:
        ...
        total_violations = len(pii_violations) + len(financial_violations) + len(sensitive_keyword_violations)
        ...
        for pii_type, pattern in self.compiled_pii_patterns.items():
            ...
            for match in matches:
                violations.append({
                    'type': pii_type,
                    'value': match,
                    'severity': 'high' if pii_type in ['ssn', 'credit_card', 'email'] else 'medium'
                })
    ...
    def _generate_recommendations(self, privacy_score: float) -> List[str]:
        ...
        if privacy_score > 0.7:
            recommendations.append("Immediate action required - significant privacy violations detected")
        elif privacy_score > 0.4:
