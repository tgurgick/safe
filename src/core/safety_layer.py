...
class SafetyLayer:
        ...
        try:
            ...
            if not text or not text.strip():
                return ValidationResult.create_invalid_result(
                    text, ["Input text cannot be empty"], (time.time() - start_time) * 1000
                )
            ...
            if user_id:
                ...
                if not rate_limit_result["allowed"]:
                    return ValidationResult.create_rate_limited_result(
                        text, rate_limit_result["reset_time"], (time.time() - start_time) * 1000
                    )
            ...
            if self.config.enable_content_filtering:
                ...
                result = ValidationResult.create_valid_result(
                    text, sanitized_text, (time.time() - start_time) * 1000
                )
            ...
            return ValidationResult.create_valid_result(
                text, text, (time.time() - start_time) * 1000
            )
        ...
        try:
            ...
            if not response or not response.strip():
                return SafetyReport.create_safe_report(
                    content_id, (time.time() - start_time) * 1000
                )
            ...
            return SafetyReport.create_safe_report(content_id, (time.time() - start_time) * 1000)
    ...
    def process_interaction(
        self, user_input: str, llm_response: str, user_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
