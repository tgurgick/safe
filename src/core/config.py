...
class SafetyConfig(BaseModel):
    ...
    safety_level: SafetyLevel = Field(
        default=SafetyLevel.MODERATE, description="Safety level for content filtering"
    )
    ...
    enable_fact_checking: bool = Field(
        default=False, description="Enable fact checking (requires external APIs)"
    )
    enable_privacy_protection: bool = Field(default=True, description="Enable privacy protection")

    # Input validation
    max_input_length: int = Field(default=1000, description="Maximum allowed input length")
    rate_limit_per_minute: int = Field(default=10, description="Rate limit per minute per user")

    # Safety thresholds
    violence_threshold: float = Field(
        default=0.7, description="Threshold for violence detection (0-1)"
    )
    hate_speech_threshold: float = Field(
        default=0.6, description="Threshold for hate speech detection (0-1)"
    )
    bias_threshold: float = Field(default=0.5, description="Threshold for bias detection (0-1)")

    # Model settings
    use_ml_models: bool = Field(
        default=False, description="Use ML models for content classification"
    )
    cache_results: bool = Field(default=True, description="Cache safety check results")

    # Monitoring
    enable_monitoring: bool = Field(default=True, description="Enable monitoring and metrics")
    log_safety_decisions: bool = Field(default=True, description="Log all safety decisions")

    # Custom rules
    custom_rules: Dict[str, Any] = Field(default_factory=dict, description="Custom safety rules")
